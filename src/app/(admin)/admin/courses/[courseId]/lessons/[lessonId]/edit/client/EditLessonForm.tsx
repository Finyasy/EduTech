...
import type { CourseForAdmin, LessonForAdmin } from "@/lib/server/data";

type EditLessonFormProps = {
  lesson: LessonForAdmin;
  courses: CourseForAdmin[];
};

export default function EditLessonForm({ lesson, courses }: EditLessonFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/admin/lesson/${lesson.id}`, {
        method: "PATCH",
      ...
      if (response.ok) {
        setResult("Lesson updated.");
        router.push(`/admin#course-${formState.courseId}`);
        router.refresh();
      } else {
        setError(data?.error ?? "Unable to update lesson.");
      }
    } catch (error) {
      setError(
